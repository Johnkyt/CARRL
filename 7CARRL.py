...
class Car:
    ...
    def draw(self, screen):
        ...
        '''
        #to display a 128x128 box around the racecar
...
for episode in tqdm(range(1, EPISODES + 1), ascii=True, unit='episodes'):
    ...
    if not episode % AGGREGATE_STATS_EVERY or episode == 1:
        ...
        if episode == 1 or min_reward >= MIN_REWARD:
            agent.model.save(f"models/{MODEL_NAME}-{int(time.time())}-{episode}-{epsilon}-{average_reward}.h5")
        

    if not episode % SAVE_MODEL_EVERY:
        agent.model.save(f"models/{MODEL_NAME}-{int(time.time())}-{episode}-{epsilon}-.h5")
