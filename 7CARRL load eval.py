...
epsilon = 0 #to evaluate the trained model
...
eval_episodes = 200 #number of episodes to run in evaluation
ep_rewards = [] #list of rewards of episodes (to get avg)
total_endpt = 0 #number of times car hit endpoint
...
class CarEnv:
    ...
    def reset(self): #reset variables that changed during each episode
        ...
        self.route_name, self.car_route = list(routes.items())[self.num]
        if self.num < 8:
            self.num += 1
        else:
            self.num = 0
...
for episode in tqdm(range(0, eval_episodes), ascii=True, unit='episodes'):
    ...
    while not done:
        ...
        if np.random.random() > epsilon:
            ...
            #print("qlist: ", agent.get_qs(current_state))
        else:
            #get random action
        ...
        '''
        if env.ncpt_index == 2:
            #so that car movement will be more random at bends
            print("TRIGGER!")
...
median_reward = statistics.median(ep_rewards)
standard_dev = statistics.pstdev(ep_rewards)
min_reward = min(ep_rewards)
max_reward = max(ep_rewards)

print("\n[ EVALUATION RESULTS ]")

print("\nMODEL:", filepath[7:]) #model name
print("EPSILON: ", epsilon)

print("\nNUM OF EPISODES: ", eval_episodes)
print("AVG REWARD: ", average_reward)
print("MEDIAN REWARD: ", median_reward)
print("STANDARD DEVIATION: ", standard_dev)
print("MIN REWARD: ", min_reward)
print("MAX REWARD: ", max_reward)
print("HIT ENDPOINT: ", total_endpt, "/", eval_episodes, " (", str((total_endpt/eval_episodes)*100) + "% )")
